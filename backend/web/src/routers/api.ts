...
router.get('/users/:name/rating.svg', async (req, res) => {
    const { name } = req.params;
    const { isHeuristic, withPerformance, withSubmissions } = req.query;
    res.setHeader('Content-Type', 'image/svg+xml');
    res.send(
        await getUserRatingImage(name, isHeuristic === 'true', withPerformance === 'true', withSubmissions === 'true'),
    );
});
router.get('/users/:name/rating.png', async (req, res) => {
    const { name } = req.params;
    const { isHeuristic, withPerformance, withSubmissions } = req.query;
    res.setHeader('Content-Type', 'image/png');
    const svgCode = await getUserRatingImage(
        name,
        isHeuristic === 'true',
...
router.get(['/users/:name/ratingGraph.svg', '/users/:name/ratingGraph.png'], async (req, res) => {
    const { name } = req.params;
    const { isHeuristic, withPerformance, withSubmissions } = req.query;
    const svg = await getHistoryImage(
        name,
        isHeuristic === 'true',
        withPerformance === 'true',
        withSubmissions === 'true',
